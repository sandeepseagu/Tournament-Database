...
def playerStandings():
    ...
    db=connect()
    c=db.cursor()
    c.execute("select id,name,n,p from standings order by n DESC;")
    rows=c.fetchall()
    db.close()
...
def swissPairings():
    ...
    while i<len(rows):
        ...
        i=i+2      #Getting next two players in the standings
        pairings.append((playerAid, playerAname,playerBid,playerAname))
